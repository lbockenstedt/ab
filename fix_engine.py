...
def _fix_pr_body(issue, repo_git, base_branch, confidence):
    """Render a fix PR's body against the fleet template.

    This used to be one sentence -- "Automated fix for issue #123. Avg
    Confidence: 87.50%" -- which left the skeptical panel judging a whole diff
    against no statement of intent at all. It rejected on exactly that ground,
    over and over, with zero code findings. The issue itself is the
    authoritative statement of intent and was sitting right here unused.

    Best-effort: PR creation must never fail because a body could not be
    rendered, so every optional part is guarded and the one-line form remains
    the fallback.
    """
    import pr_template
    ...
    try:
        intent = pr_template.from_issue(
            getattr(issue, "number", None), getattr(issue, "title", "") or "",
            getattr(issue, "body", "") or "", getattr(issue, "html_url", None))
    except Exception:  # noqa: BLE001
        intent = "Fixes #%s." % getattr(issue, "number", "?")

    changed = []
    try:
        diff = repo_git.git.diff("--numstat", "origin/%s...HEAD" % base_branch)
        for line in (diff or "").splitlines():
            cols = line.split("\t")
            if len(cols) == 3:
                changed.append("- `%s` (+%s/-%s)" % (cols[2], cols[0], cols[1]))
    except Exception:  # noqa: BLE001
        pass

    if changed:
        shown = changed[:40]
        solution = "AppBuilder generated this fix. Files changed (%d):\n\n%s" % (
            len(changed), "\n".join(shown))
        if len(changed) > len(shown):
            solution += "\n- _...and %d more._" % (len(changed) - len(shown))
    else:
        solution = "AppBuilder generated this fix for the issue quoted above."

    guardrails = (
        "Generated by AppBuilder's fix engine and gated by its guardrail "
        "checks (boundary paths, secrets, force-push policy) before this PR "
        "was opened. Targets `%s`." % base_branch)
    verification = (
        "Reviewed by the cross-provider skeptical panel during generation; "
        "average confidence **%.2f%%**. Repository CI runs on this PR."
        % (float(confidence or 0.0) * 100.0))

    try:
        return pr_template.render(intent=intent, solution=solution,
                                  guardrails=guardrails, verification=verification)
    except Exception:  # noqa: BLE001
        return "Automated fix for issue #%s. Avg Confidence: %.2f%%" % (
            getattr(issue, "number", "?"), float(confidence or 0.0) * 100.0)
                ...
                if existing_pr:
                    ...
                    try:
                        pr = repo_obj.create_pull(
                            ...
                            body=_fix_pr_body(issue, repo_git, base_branch,
                                              final_confidence),
                            head=target_branch,
                            base=base_branch
                        )
