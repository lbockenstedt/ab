...
- **Promotion (`routes.py`):** `GET /api/promote/repos` (repos the buttons can target — `monitored_repos` plus the self-diagnosis repo, which is the default selection) and `POST /api/promote` (`{repo, source, target}`) which dispatches that repo's `.github/workflows/promote.yml`. See *Branch promotion buttons* below.
...
- **Branch promotion buttons (WebUI footer)** — three buttons in the footer action bar, beside *Hub Update* / *Blackout*, with a repo selector (populated from `GET /api/promote/repos`; defaults to AppBuilder's own self-diagnosis repo):
